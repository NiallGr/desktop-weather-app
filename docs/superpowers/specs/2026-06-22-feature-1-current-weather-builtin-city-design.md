...
- **Open findings (gating):**
  ...
  4. *(seam-cynicism, minor)* Spec `## Seam inventory` has no row for the host-OS/runtime + NodaTime-TZDB facet: `ObservedAt` correctness depends on `timezone=auto` returning an IANA name resolvable in NodaTime's bundled TZDB. The null-zone failure path is handled in the Plan, but the boundary (drifts with a tzdb-version change) is folded silently into the network read rather than named.
- **Next step:** The fix is principally **grounding the seam**, not rewriting prose. Add `api.open-meteo.com` to the environment egress allowlist, capture a real `/v1/forecast` response, verify the field names / types / `time` format against it, replace `Fixtures/london-current.json` with the captured fixture, mark Seam 1 `(e)` grounded, and add the missing TZDB inventory row (finding 4). Then re-run `/feature-doc-gauntlet` from a fresh session. `enate-to-stories` must not break this Feature into stories until the sign-off shows `Result: pass`.
